...
$(function () {
    ...
    //세로 스크롤에 따라 배경색과 글자색 clip-path변화 애니메이션
    //gsap타임라인을 만든다. 스크롤 애니메이션 순서표
    gsap.timeline({
        scrollTrigger: {
            //스크롤을 감지할 대상:
            trigger: '#con3',
            start: '0% 80%', //#con3 맨 위(0%)가 화면의 80% 지점에 닿을 때 애니시작
            end: '100% 100%', //#con3의 맨 아래(100%)가 화면의 맨 아래 (100%)에 닿을 때 애니 끝
            scrub: 1, //스크롤 움직임에 따라서 애니메이션도 같이 움직이게 함
        }
    })
        .to('#con3', {
            backgroundColor: '#fff',
            color: '#000',
            duration: 5,
            ease: 'none'//애니메이션이 부드럽게 변하지 않고 스크롤에 딱 맞게 움직이게 설정
        }, 0)
        .fromTo('#con3 .videoWrap video', { 'clip-path': 'inset(60% 60% 60% 60% round 30%)' }, { 'clip-path': 'inset(0% 0% 0% 0% round 0%)', duration: 3, ease: 'none' }, 0)
    ...
    animations.forEach((anim, index) => {
        ani[index] = gsap.to(anim.target, {
            ...
            scrollTrigger: {
                ...
                start: 'left center',
                toggleActions: "play none reverse none", //한 번 재생하고 뒤로 갈 때만 역재생
    ...
    //article요소들에 부드럽게 나타나는 애니메이션 추가
    ...
    con5Article.forEach((el, i) => {
        gsap.timeline({
            scrollTrigger: {
                ...
                end: 'top 20%', //화면 위쪽으로 거의 다 올라왔을 때 끝
